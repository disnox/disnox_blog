...
-- 修改用户名与命名
UPDATE `user` SET `username`='nox',`password`='a12345678` WHERE id = 1
```
...
![img](https://img.nox.cn/20201009150524563.png)
...
SELECT LOWER('nox') --到小写
SELECT UPPER('nox') --到大写
```
...
CREATE USER nox IDENTIFIED BY '123456'

-- 修改密码 (修改当前用户)
SET PASSWORD = PASSWORD('a123456')

-- 修改密码 (修改指定用户)
SET PASSWORD FOR nox= PASSWORD('a123456')

-- 重命名
RENAME USER nox TO zeyu

-- 用户授权
-- CRANT ALL PRIVILEGES ON 库.表 TO 用户
-- ALL PRIVILEGES 除了给别人授权(GRANT),其他权限都有
CRANT ALL PRIVILEGES ON *.* TO nox
...
REVOKE ALL PRIVILEGES ON *.* FROM nox
...
![image-20210102213536190](https://img.nox.cn/image-20210102213536190.png)
