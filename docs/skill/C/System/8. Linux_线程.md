...
Linux线程是一种程序执行的方式，允许多个执行流在单个进程中并发执行。这些执行流称为线程，并且它们共享进程中的所有资源，包括内存和打开的文件。因此，多线程可以让程序更有效地利用多核处理器的计算能力，并且可以程序同时执行多个任务。
...
pthread_create是Linux线程库中的一个函数，用于创建新的线程。它需要四个参数：
...
pthread_create函数的返回值是一个整数，表示创建线程的结果。如果创建成功，则返回0，如果创建失败，则返回错误代码。
...
pthread_exit( ) 是 Linux 线程库中的一个函数，允许线程终止并返回值给调用者。
...
pthread_join( ) 是 Linux 线程库中的一个函数，它允许一个线程等待另一个线程完成，并获取该线程的返回值。它有两个参数：
...
pthread_attr_setdetachstate( ) 用于设置线程的分离状态，分离状态指定线程是否是分离的，即它是否在终止时自动释放它所占用的资源，而不需要等待其他线程进行 join 操作。

>  pthread_attr_setdetachstate函数需要两个参数：
...
pthread_attr_getdetachstate( ) 函数用于获取线程属性对象中的分离状态。该函数需要两个参数：
...
int pthread_setcanceltype(int type, int *oldtype);		// 是否立即响应(默认延迟延迟相应)
```

pthread_cancel( ) 用于取消指定的线程。该函数需要一个参数：    
